...
export function TransactionsPage({ transactions, accounts, categories, currency, selectedMonthLabel, onImport, onEdit, onDuplicate, onToggleStatus, onDelete, onNewTransaction }: TransactionsPageProps) {
  ...
  const importCsv = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      setImportMessage(`${parsed.length} transações prontas para revisão.`);
    } catch (err) {
  ...
  const confirmImport = async () => {
    ...
    try {
      ...
      setImportMessage(`${importPreview.length} transações importadas.`);
      setImportPreview([]);
    } catch (err) {
      setImportMessage(err instanceof Error ? err.message : "Não foi possível importar o arquivo.");
    } finally {
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 max-w-[1100px] mx-auto space-y-4 animate-fade-in">
      <div className="bg-card border border-border rounded-2xl p-5">
        <div>
          <p className="text-[11px] font-medium text-gray-400 uppercase">Lançamentos</p>
      ...
      {importPreview.length > 0 && (
        <div className="bg-card border border-border rounded-2xl overflow-hidden">
          <div className="px-5 py-4 border-b border-border flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            <div>
              <h3 className="text-sm font-semibold text-gray-900">Prévia da importação</h3>
              <p className="text-[11px] text-gray-400 mt-0.5">{importPreview.length} transações aguardando confirmação</p>
            </div>
            <div className="flex gap-2">
              <button onClick={() => setImportPreview([])} className="border border-border px-3 py-2 rounded-xl text-[12px] font-semibold text-gray-600 hover:bg-gray-50">Cancelar</button>
              <button onClick={() => void confirmImport()} disabled={importing} className="bg-gray-900 text-white px-3 py-2 rounded-xl text-[12px] font-semibold hover:bg-gray-800 disabled:opacity-50">Confirmar importação</button>
            </div>
          </div>
          <div className="divide-y divide-border max-h-[280px] overflow-y-auto">
            {importPreview.slice(0, 8).map((transaction, index) => (
              <div key={`${transaction.date}-${transaction.description}-${index}`} className="px-5 py-3 flex items-center justify-between gap-3">
                <div className="min-w-0">
                  <p className="text-[13px] font-semibold text-gray-900 truncate">{transaction.description}</p>
                  <p className="text-[11px] text-gray-400">{transaction.category} · {transaction.date} · {transaction.status === "pending" ? "Pendente" : "Concluída"}</p>
                </div>
                ...
                <div key={transaction.id} className={cn("grid grid-cols-[1fr_auto] sm:grid-cols-[1fr_120px_120px_120px_144px] gap-3 sm:gap-4 px-4 sm:px-5 py-4 items-center transition-all", deletingId === transaction.id ? "opacity-30 scale-[0.99]" : "hover:bg-gray-50/50")}>
                  <div className="flex items-center gap-3 min-w-0">
                    ...
                    <div className="min-w-0">
                      ...
                      <div className="flex flex-wrap items-center gap-2 mt-0.5 sm:hidden"><span className="text-[11px] text-gray-400">{transaction.category}</span><span className="text-[11px] text-gray-400">{transaction.accountId ? accountNameById.get(transaction.accountId) ?? "Conta" : "Sem conta"}</span><span className="text-[11px] text-gray-400">{formatDateFull(transaction.date)}</span></div>
