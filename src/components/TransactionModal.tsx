...
export function TransactionModal({ categories, open, onClose, onSubmit }: TransactionModalProps) {
  ...
  const submit = async () => {
    ...
    setSaving(false);
    setForm({ description: "", amount: "", type: "expense", category: "Moradia", status: "completed", date: new Date().toISOString().split("T")[0] });
    onClose();
  };
    ...
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-end sm:items-center justify-center z-50 sm:p-4 animate-fade-in" onClick={onClose}>
      <div className="bg-card w-full sm:max-w-[440px] sm:border sm:border-border rounded-t-2xl sm:rounded-2xl shadow-2xl max-h-[92vh] flex flex-col animate-slide-up" onClick={event => event.stopPropagation()}>
        <div className="flex items-center justify-between p-5 border-b border-border flex-shrink-0">
          <div>
            <h2 className="text-[15px] font-semibold text-gray-900">Nova transação</h2>
            <p className="text-[11px] text-gray-400 mt-0.5">Registre uma movimentação</p>
          </div>
